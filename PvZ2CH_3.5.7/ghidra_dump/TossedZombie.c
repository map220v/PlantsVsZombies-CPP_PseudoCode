// Class: TossedZombie


/* TossedZombie::~TossedZombie() */

void __thiscall TossedZombie::~TossedZombie(TossedZombie *this)

{
  Sexy::RtWeakPtr<Sexy::ResourceInfo>::~RtWeakPtr((RtWeakPtr<Sexy::ResourceInfo> *)(this + 0xe8));
  RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
  ::~RtReflectionDelegate
            ((RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
              *)(this + 0xa0));
  RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
  ::~RtReflectionDelegate
            ((RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
              *)(this + 0x48));
  Sexy::RtWeakPtr<Sexy::ResourceInfo>::~RtWeakPtr((RtWeakPtr<Sexy::ResourceInfo> *)(this + 0x40));
  return;
}


void __thiscall TossedZombie::~TossedZombie(TossedZombie *this)

{
  Sexy::RtWeakPtr<Sexy::ResourceInfo>::~RtWeakPtr((RtWeakPtr<Sexy::ResourceInfo> *)(this + 0xe8));
  RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
  ::~RtReflectionDelegate
            ((RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
              *)(this + 0xa0));
  RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
  ::~RtReflectionDelegate
            ((RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
              *)(this + 0x48));
  Sexy::RtWeakPtr<Sexy::ResourceInfo>::~RtWeakPtr((RtWeakPtr<Sexy::ResourceInfo> *)(this + 0x40));
  return;
}


void __thiscall TossedZombie::~TossedZombie(TossedZombie *this)

{
  Sexy::RtWeakPtr<Sexy::ResourceInfo>::~RtWeakPtr((RtWeakPtr<Sexy::ResourceInfo> *)(this + 0xe8));
  RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
  ::~RtReflectionDelegate
            ((RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
              *)(this + 0xa0));
  RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
  ::~RtReflectionDelegate
            ((RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
              *)(this + 0x48));
  Sexy::RtWeakPtr<Sexy::ResourceInfo>::~RtWeakPtr((RtWeakPtr<Sexy::ResourceInfo> *)(this + 0x40));
  return;
}


void __thiscall TossedZombie::~TossedZombie(TossedZombie *this)

{
  Sexy::RtWeakPtr<Sexy::ResourceInfo>::~RtWeakPtr((RtWeakPtr<Sexy::ResourceInfo> *)(this + 0xe8));
  RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
  ::~RtReflectionDelegate
            ((RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
              *)(this + 0xa0));
  RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
  ::~RtReflectionDelegate
            ((RtReflectionDelegate<Sexy::Delegate4<std::string_const&,float,std::string_const&,std::string_const&>>
              *)(this + 0x48));
  Sexy::RtWeakPtr<Sexy::ResourceInfo>::~RtWeakPtr((RtWeakPtr<Sexy::ResourceInfo> *)(this + 0x40));
  return;
}


/* TossedZombie::TossedZombie() */

void __thiscall TossedZombie::TossedZombie(TossedZombie *this)

{
  DVec3::DVec3((DVec3 *)this);
  DVec3::DVec3((DVec3 *)(this + 0xc));
  DVec3::DVec3((DVec3 *)(this + 0x18));
  DVec3::DVec3((DVec3 *)(this + 0x24));
  DVec3::DVec3((DVec3 *)(this + 0x30));
  std::__detail::_Hash_node<std::pair<Sexy::RtName_const,Sexy::RtId::Uid>,true>::_Hash_node
            ((_Hash_node<std::pair<Sexy::RtName_const,Sexy::RtId::Uid>,true> *)(this + 0x40));
  RtReflectionDelegate<Sexy::Delegate1<Zombie*>>::RtReflectionDelegate
            ((RtReflectionDelegate<Sexy::Delegate1<Zombie*>> *)(this + 0x48));
  RtReflectionDelegate<Sexy::Delegate1<Zombie*>>::RtReflectionDelegate
            ((RtReflectionDelegate<Sexy::Delegate1<Zombie*>> *)(this + 0xa0));
  std::__detail::_Hash_node<std::pair<Sexy::RtName_const,Sexy::RtId::Uid>,true>::_Hash_node
            ((_Hash_node<std::pair<Sexy::RtName_const,Sexy::RtId::Uid>,true> *)(this + 0xe8));
  return;
}


/* TossedZombie::TossedZombie(TossedZombie const&) */

void __thiscall TossedZombie::TossedZombie(TossedZombie *this,TossedZombie *param_1)

{
  undefined4 uVar1;
  undefined4 uVar2;
  TossedZombie TVar3;
  TossedZombie TVar4;
  
  *(undefined8 *)this = *(undefined8 *)param_1;
  *(undefined4 *)(this + 8) = *(undefined4 *)(param_1 + 8);
  *(undefined8 *)(this + 0xc) = *(undefined8 *)(param_1 + 0xc);
  *(undefined4 *)(this + 0x14) = *(undefined4 *)(param_1 + 0x14);
  *(undefined8 *)(this + 0x18) = *(undefined8 *)(param_1 + 0x18);
  *(undefined4 *)(this + 0x20) = *(undefined4 *)(param_1 + 0x20);
  *(undefined8 *)(this + 0x24) = *(undefined8 *)(param_1 + 0x24);
  *(undefined4 *)(this + 0x2c) = *(undefined4 *)(param_1 + 0x2c);
  *(undefined8 *)(this + 0x30) = *(undefined8 *)(param_1 + 0x30);
  *(undefined4 *)(this + 0x38) = *(undefined4 *)(param_1 + 0x38);
  *(undefined4 *)(this + 0x3c) = *(undefined4 *)(param_1 + 0x3c);
  Sexy::RtWeakPtr<Sexy::SoundResource>::RtWeakPtr
            ((RtWeakPtr<Sexy::SoundResource> *)(this + 0x40),(RtWeakPtrBase *)(param_1 + 0x40));
  RtReflectionDelegate<Sexy::Delegate1<Zombie*>>::RtReflectionDelegate
            ((RtReflectionDelegate<Sexy::Delegate1<Zombie*>> *)(this + 0x48),
             (RtReflectionDelegate *)(param_1 + 0x48));
  uVar1 = *(undefined4 *)(param_1 + 0x94);
  uVar2 = *(undefined4 *)(param_1 + 0x98);
  TVar3 = param_1[0x91];
  TVar4 = param_1[0x92];
  this[0x90] = param_1[0x90];
  this[0x91] = TVar3;
  this[0x92] = TVar4;
  *(undefined4 *)(this + 0x94) = uVar1;
  *(undefined4 *)(this + 0x98) = uVar2;
  RtReflectionDelegate<Sexy::Delegate1<Zombie*>>::RtReflectionDelegate
            ((RtReflectionDelegate<Sexy::Delegate1<Zombie*>> *)(this + 0xa0),
             (RtReflectionDelegate *)(param_1 + 0xa0));
  Sexy::RtWeakPtr<Sexy::SoundResource>::RtWeakPtr
            ((RtWeakPtr<Sexy::SoundResource> *)(this + 0xe8),(RtWeakPtrBase *)(param_1 + 0xe8));
  return;
}


/* TossedZombie::TEMPNAMEPLACEHOLDERVALUE(TossedZombie&&) */

TossedZombie * __thiscall TossedZombie::operator=(TossedZombie *this,TossedZombie *param_1)

{
  undefined4 uVar1;
  undefined4 uVar2;
  TossedZombie TVar3;
  TossedZombie TVar4;
  
  Sexy::SexyVector3::operator=((SexyVector3 *)this,(SexyVector3 *)param_1);
  Sexy::SexyVector3::operator=((SexyVector3 *)(this + 0xc),(SexyVector3 *)(param_1 + 0xc));
  Sexy::SexyVector3::operator=((SexyVector3 *)(this + 0x18),(SexyVector3 *)(param_1 + 0x18));
  Sexy::SexyVector3::operator=((SexyVector3 *)(this + 0x24),(SexyVector3 *)(param_1 + 0x24));
  Sexy::SexyVector3::operator=((SexyVector3 *)(this + 0x30),(SexyVector3 *)(param_1 + 0x30));
  *(undefined4 *)(this + 0x3c) = *(undefined4 *)(param_1 + 0x3c);
  Sexy::RtWeakPtr<PowerPropertySheet>::operator=
            ((RtWeakPtr<PowerPropertySheet> *)(this + 0x40),(RtWeakPtr *)(param_1 + 0x40));
  RtReflectionDelegate<Sexy::Delegate1<Plant*>>::operator=
            ((RtReflectionDelegate<Sexy::Delegate1<Plant*>> *)(this + 0x48),
             (RtReflectionDelegate *)(param_1 + 0x48));
  uVar1 = *(undefined4 *)(param_1 + 0x94);
  uVar2 = *(undefined4 *)(param_1 + 0x98);
  TVar3 = param_1[0x92];
  TVar4 = param_1[0x91];
  this[0x90] = param_1[0x90];
  this[0x92] = TVar3;
  this[0x91] = TVar4;
  *(undefined4 *)(this + 0x94) = uVar1;
  *(undefined4 *)(this + 0x98) = uVar2;
  RtReflectionDelegate<Sexy::Delegate1<Plant*>>::operator=
            ((RtReflectionDelegate<Sexy::Delegate1<Plant*>> *)(this + 0xa0),
             (RtReflectionDelegate *)(param_1 + 0xa0));
  Sexy::RtWeakPtr<PowerPropertySheet>::operator=
            ((RtWeakPtr<PowerPropertySheet> *)(this + 0xe8),(RtWeakPtr *)(param_1 + 0xe8));
  return this;
}


/* TossedZombie::TEMPNAMEPLACEHOLDERVALUE(TossedZombie const&) */

TossedZombie * __thiscall TossedZombie::operator=(TossedZombie *this,TossedZombie *param_1)

{
  undefined4 uVar1;
  undefined4 uVar2;
  TossedZombie TVar3;
  TossedZombie TVar4;
  
  Sexy::SexyVector3::operator=((SexyVector3 *)this,(SexyVector3 *)param_1);
  Sexy::SexyVector3::operator=((SexyVector3 *)(this + 0xc),(SexyVector3 *)(param_1 + 0xc));
  Sexy::SexyVector3::operator=((SexyVector3 *)(this + 0x18),(SexyVector3 *)(param_1 + 0x18));
  Sexy::SexyVector3::operator=((SexyVector3 *)(this + 0x24),(SexyVector3 *)(param_1 + 0x24));
  Sexy::SexyVector3::operator=((SexyVector3 *)(this + 0x30),(SexyVector3 *)(param_1 + 0x30));
  *(undefined4 *)(this + 0x3c) = *(undefined4 *)(param_1 + 0x3c);
  Sexy::RtWeakPtr<PowerPropertySheet>::operator=
            ((RtWeakPtr<PowerPropertySheet> *)(this + 0x40),(RtWeakPtr *)(param_1 + 0x40));
  RtReflectionDelegate<Sexy::Delegate1<RealObject*>>::operator=
            ((RtReflectionDelegate<Sexy::Delegate1<RealObject*>> *)(this + 0x48),
             (RtReflectionDelegate *)(param_1 + 0x48));
  uVar1 = *(undefined4 *)(param_1 + 0x94);
  uVar2 = *(undefined4 *)(param_1 + 0x98);
  TVar3 = param_1[0x92];
  TVar4 = param_1[0x91];
  this[0x90] = param_1[0x90];
  this[0x92] = TVar3;
  this[0x91] = TVar4;
  *(undefined4 *)(this + 0x94) = uVar1;
  *(undefined4 *)(this + 0x98) = uVar2;
  RtReflectionDelegate<Sexy::Delegate1<RealObject*>>::operator=
            ((RtReflectionDelegate<Sexy::Delegate1<RealObject*>> *)(this + 0xa0),
             (RtReflectionDelegate *)(param_1 + 0xa0));
  Sexy::RtWeakPtr<PowerPropertySheet>::operator=
            ((RtWeakPtr<PowerPropertySheet> *)(this + 0xe8),(RtWeakPtr *)(param_1 + 0xe8));
  return this;
}


/* TossedZombie::TossedZombie(TossedZombie&&) */

void __thiscall TossedZombie::TossedZombie(TossedZombie *this,TossedZombie *param_1)

{
  undefined4 uVar1;
  undefined4 uVar2;
  TossedZombie TVar3;
  TossedZombie TVar4;
  
  *(undefined8 *)this = *(undefined8 *)param_1;
  *(undefined4 *)(this + 8) = *(undefined4 *)(param_1 + 8);
  *(undefined8 *)(this + 0xc) = *(undefined8 *)(param_1 + 0xc);
  *(undefined4 *)(this + 0x14) = *(undefined4 *)(param_1 + 0x14);
  *(undefined8 *)(this + 0x18) = *(undefined8 *)(param_1 + 0x18);
  *(undefined4 *)(this + 0x20) = *(undefined4 *)(param_1 + 0x20);
  *(undefined8 *)(this + 0x24) = *(undefined8 *)(param_1 + 0x24);
  *(undefined4 *)(this + 0x2c) = *(undefined4 *)(param_1 + 0x2c);
  *(undefined8 *)(this + 0x30) = *(undefined8 *)(param_1 + 0x30);
  *(undefined4 *)(this + 0x38) = *(undefined4 *)(param_1 + 0x38);
  *(undefined4 *)(this + 0x3c) = *(undefined4 *)(param_1 + 0x3c);
  Sexy::RtWeakPtr<Sexy::SoundResource>::RtWeakPtr
            ((RtWeakPtr<Sexy::SoundResource> *)(this + 0x40),(RtWeakPtrBase *)(param_1 + 0x40));
  RtReflectionDelegate<Sexy::Delegate1<Zombie*>>::RtReflectionDelegate
            ((RtReflectionDelegate<Sexy::Delegate1<Zombie*>> *)(this + 0x48),
             (RtReflectionDelegate *)(param_1 + 0x48));
  uVar1 = *(undefined4 *)(param_1 + 0x94);
  uVar2 = *(undefined4 *)(param_1 + 0x98);
  TVar3 = param_1[0x91];
  TVar4 = param_1[0x92];
  this[0x90] = param_1[0x90];
  this[0x91] = TVar3;
  this[0x92] = TVar4;
  *(undefined4 *)(this + 0x94) = uVar1;
  *(undefined4 *)(this + 0x98) = uVar2;
  RtReflectionDelegate<Sexy::Delegate1<Zombie*>>::RtReflectionDelegate
            ((RtReflectionDelegate<Sexy::Delegate1<Zombie*>> *)(this + 0xa0),
             (RtReflectionDelegate *)(param_1 + 0xa0));
  Sexy::RtWeakPtr<Sexy::SoundResource>::RtWeakPtr
            ((RtWeakPtr<Sexy::SoundResource> *)(this + 0xe8),(RtWeakPtrBase *)(param_1 + 0xe8));
  return;
}

